...
class WP_Bootstrap_Comments_Walker extends Walker_Comment {
    ...
    protected function html5_comment( $comment, $depth, $args, $is_parent = false )
    {
        ...
        $comment_classes[] = 'media comments__item';
        ...
        <<?php echo $tag; ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( $class_str, $comment ); ?>>
            ...
            <div id="div-comment-<?php comment_ID(); ?>" class="">
                ...
                <?php if ( 0 != $args['avatar_size'] && 'pingback' !== $type && 'trackback' !== $type ) { ?>
                    <div class="author">
						...
						<div class="author__info">
							<?php printf( __( '%s' ), sprintf( '<h4 class="author__name">%s</h4>', get_comment_author_link( $comment ) ) ); ?>
							<time class="author__date" datetime="<?php comment_time( 'c' ); ?>">
								<?php
								/* translators: 1: comment date */
								printf( __( '%1$s' ), get_comment_date( 'j F Y', $comment ) );
								?>
                            </time>
				...
				<div class="comments__functions _close-comments">
					<div class="replay"><?php $this->comment_reply_link( $comment, $depth, $args, $add_below = 'Ответить' ); ?></div>
				</div>
                <?php }; ?>
